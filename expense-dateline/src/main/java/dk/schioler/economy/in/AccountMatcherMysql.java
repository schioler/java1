...
import dk.schioler.economy.PatternMatch;
...
public class AccountMatcherMysql implements AccountMatcher {
   ...
   static final String SQL_SELECT = "Select id,account_id, pattern from PATTERN_MATCH";
...
//   }
   ...
   private static final class PatternMapper implements RowMapper<PatternMatch> {
      public PatternMatch mapRow(ResultSet rs, int rowNum) throws SQLException {
         PatternMatch pm = new PatternMatch(rs.getLong("id"), rs.getLong("account_id"), rs.getString("pattern"));
         return pm;
      }
   }
   ...
   String SQL_FROM_ACCONT = "SELECT id FROM ACCOUNT WHERE TYPE = " + "'acc'" + "and  locate(NAME,?) > 0";
   ...
   public long matchText(String text) {
      ...
      args[0] = txtUpper;
      List<PatternMatch> pms = this.jdbcTemplate.query(SQL_DIRECT, args, new PatternMapper());
      if (pms.size() == 0) {         
